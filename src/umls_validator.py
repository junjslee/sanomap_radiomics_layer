...
NER (currently OpenMed-NER-SpeciesDetect-PubMed-335M; the prior
`d4data/biomedical-ner-all` is retained as a fallback selectable via the
`--microbe-ner-model-id` CLI flag in ``text_ner_minerva.py``) to remove
gene-function, phenotype, and host-organism noise that NER mislabels as
Microbe.
