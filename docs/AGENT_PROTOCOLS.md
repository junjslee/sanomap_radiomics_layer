...
1. **Context Awareness:** Always refer to `MINERVA_PIPELINE.md`, `RADIOMICS_LAYER_SPECS.md`, and `RUN_CONTEXT.md` before writing logic.
...
6. **Direct Evidence Only:** Do not emit graph edges from within-paper bridge matches that only share a disease context. Such matches must be written as audit-only hypotheses and never ingested into Neo4j as asserted relationships.
