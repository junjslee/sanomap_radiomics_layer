...
\begin{itemize}
    ...
    \item \textbf{2} \texttt{CORRELATES\_WITH} edges from Vision Track: (1) \textit{Prevotella nigrescens} $\leftrightarrow$ GLCM\_Correlation on CT, $r = 0.95$ (distance metric = 0.05, support fraction = 1.0), verified against figure PMC10605408; (2) \textit{Firmicutes} $\leftrightarrow$ Total fat \%, $r = -0.95$ (nearest verified $r = -0.906$, support fraction = 1.0), verified against figure PMC6178902
    \item \textbf{18} \texttt{BodyLocation} nodes, \textbf{5} \texttt{ImagingModality} nodes
    \item \textbf{184} total Neo4j export rows; \textbf{156} automated test checks passing
\end{itemize}
