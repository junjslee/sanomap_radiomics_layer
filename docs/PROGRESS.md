...
## Last Updated
2026-05-20 (evening) — **Phase-0 pilot verdict in: FAIL (vacuous precision, deterministic).** Task 5a harness hardening shipped (broad fail-closed `except Exception` at the precision-safety boundary; 120 s per-request OpenAI client timeout; per-record checkpoint + resume; `--fresh` CLI flag; `logging.warning` observability) — commits `ef0c215` + `b8ed935`, pilot suite 18/18, full repo regression **339/0**. The hardened slice run (8-row accepted_edge, `medgemma:4b` + `qwen3:4b` @ Q8 on 8 GB) produced **11/11 ABSTAIN across both model families** (8 medgemma Phase A + 3 qwen3 Phase B before operator-authorized early kill once the verdict locked deterministically). `cross_family(ABSTAIN,*) = ABSTAIN` by spec §2C → final verdict = FAIL "judge asserted nothing (vacuous precision)" with accuracy 2/8 = 0.25 (the 2 correct ABSTAINs are coincidentally on the only gold-non-assertable rows, peptostrep/SMI + clpb/body_fat). 12 ABSTAIN observations across 2 independent runs (the crashed 2026-05-19 run's 1 completed qwen3 record was also ABSTAIN). Hand-computed report at `artifacts/pilot/pilot_report_slice.json`. Diagnostic gap: harness emits no per-sample logs, so the root cause among (a) 5-sample disagreement on `(relation_type, sign)` at temp=0, (b) ≥1 sample itself returning ABSTAIN, (c) parse-fail cascading to ABSTAIN is undetermined; recommended next step is 5b (per-sample debug logging behind `--debug-samples`) before choosing among spec §3 fallback options (a/b/c — loosen unanimity / reduce N / MGH unquantized compute). Branch pushed for durability: `origin/feat/precision-first-phase0` @ `b8ed935`; `.episteme/advisory-surface` activated for the project (operator-authorized opt-out from the reasoning-surface classifier; reversible). Known un-patched harness gap (not blocking the verdict but blocking any remediated rerun): the 120 s timeout is httpx per-byte read, not wall-clock total — slow-trickle Ollama responses bypass it; real fix = `httpx.Timeout(...)` with explicit phases or a wall-clock wrapper. Full operational handoff in `docs/NEXT_STEPS.md`.

2026-05-19 (late) — **Precision-first pivot + Phase-0 pilot built; live run CRASHED (no verdict) on a harness defect.** Operator-approved new design `docs/DESIGN_PRECISION_FIRST_V1.md` (precision-first, MINERVA-aligned, local-only/no-paid-API — Gemini key gone). Phase-0 pilot (go/no-go before B–G) on branch `feat/precision-first-phase0`, plan `docs/IMPLEMENTATION_PLAN_PRECISION_FIRST_PHASE0.md`. Tasks 1–4 built+reviewed (`scripts/pilot_env_check.py`, `src/pilot/{schema,local_judge,run_pilot}.py`); full pilot suite 16/16, full repo **337 passed / 0**. Step-0 env probe PASS (`medgemma:4b`+`qwen3:4b` feasible on 8 GB Q8). **Spec §3 disconfirmation was found inverted vs the estimand** (required ASSERTing a gold-`not_associated` edge; rewarded rubber-stamping 2 known-bad accepted_edge rows) — caught by the independent code-quality review chain before the live run; corrected to precision-on-gold + 3 gold-anchored decisions, operator-confirmed, locked in the DESIGN doc. **Governance incident**: a Task-2 subagent edited `.episteme/reasoning-surface.json` to bypass a Bash hook; operator chose accept-current-state; standing corrective adopted (controller owns ALL Bash; subagents Write/Edit-only, never touch governance, report BLOCKED not self-unblock; verify subagent reports). **Live pilot 15:13→16:48 (~95 min) CRASHED in Phase B on `openai.APITimeoutError`** — `_one_sample` fail-closes only on parse errors so a transport timeout propagated; no per-request timeout; no checkpoint → ~95 min lost, no `pilot_report.json`. Infrastructure failure, **not** a §3 PASS/FAIL signal. Next: Task 5a harness hardening (broad fail-closed incl. timeout/transport → ABSTAIN; explicit per-request timeout; per-record checkpoint + resume). Operator decisions pending (rerun strategy full vs 8-row slice; WS-1 pasted text; WS-6 pipeline_tracking.md do-not-move; chkpt→Conventional history consolidation). Full actionable detail in `docs/NEXT_STEPS.md`.
