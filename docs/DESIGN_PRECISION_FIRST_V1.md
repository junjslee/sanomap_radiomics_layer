...
- **Disconfirmation condition (estimand-aligned; operator-confirmed 2026-05-19, replacing the original inverted predicate):** scored as judge-decision vs **gold Pass-1 label** on the `accepted_edge` stratum. `correct ASSERT` ⟺ gold ∈ {associated_positive, associated_negative, associated_unsigned}; `correct ABSTAIN` ⟺ gold ∈ {not_associated, unclear, no_association_explicit}. **PASS iff (1)** precision = correct-ASSERTs / total-ASSERTs ≥ **0.90** (§2-D target) with ≥ 1 ASSERT (non-vacuous), **and (2)** all three gold-anchored thesis decisions are correct: ASSERT `eubacterium → visceral_adipose_tissue` (gold *associated_positive*); ASSERT `ruminococcus → sarcopenia` (gold *associated_negative*); **ABSTAIN** `peptostreptococcus stomatis → skeletal_muscle_index` (gold *not_associated*). **Why the original was retired:** the first-cut "(i) all 3 closers ASSERT / (ii) ≥5 of 8 accepted retained" predicate was inverted vs the precision-first estimand — it rewarded rubber-stamping and *required asserting a gold-negative edge* (peptostrep→SMI is gold `not_associated`); 2 of the 8 accepted_edge rows are known-bad (the Edge-#5 entity error `gut bacterial clpb-like gene function`; the `bacteriodetes` typo, UMLS-dropped). Surfaced by the Task-4 code-quality review + controller verification of the real gold schema. On FAIL → fallback ladder: (a) raise N / tighten abstention; (b) defer to MGH unquantized compute (G makes it a rerun, not a rebuild); (c) consciously scope the graph smaller, stated in Limitations. Quantify *before* investing in B–G.
